...
function onPartialResult(progress, result) {
	...
	if (split.think.length != 0) {
		...
		think = '<div class="mb-3">' +
			'<button class="btn btn-secondary" type="button" data-bs-toggle="collapse" data-bs-target="#think' + sentMessages + '" aria-expanded="' + prevThink + '" aria-controls="think' + sentMessages + '">Show/hide thought process</button>' +
			'<div class="collapse' + (prevThink ? ' show' : '') + ' p-2" id="think' + sentMessages + '" style="border: 1px solid white; width: auto;">' + marked.parse(split.think) + '</div></div>';
	}
